...
- `data/app.db` — каталог задач (создаётся автоматически из `schema.sql` при первом запуске)
- `data/datasets/task_<id>.db` — отдельная база данных для каждой задачи
  - создаётся автоматически при первом открытии задачи
  - кнопка "Сбросить датасет" удаляет файл и пересоздаёт
